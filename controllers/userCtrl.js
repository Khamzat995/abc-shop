...
const userCtrl = {
    register: async (req, res) =>{
        try {
            ...
            if(user) return res.status(400).json({msg: "Такая электронная почта уже существует."})

            if(password.length < 6) 
                return res.status(400).json({msg: "Пароль должен состоять не менее чем из 6 символов"})
    ...
    addCart: async (req, res) =>{
        try {
            ...
            return res.json({msg: "Добавлен в корзину"})
        } catch (err) {
            return res.status(500).json({msg: err.message})
        }
    },
